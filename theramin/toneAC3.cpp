...
//
// See "toneAC3.h" for purpose, syntax, version history, links, and more.
// ---------------------------------------------------------------------------
...
unsigned long _tAC3_time; // Used to track end note with timer when playing note in the background.
...
int _tAC3_prescale[] = { 2, 16, 64, 128, 256, 512, 2048 }; // Prescaler.

void toneAC3(uint8_t pin1, uint8_t pin2, unsigned int frequency, unsigned long length, uint8_t background) {
  long top;
  uint8_t prescaler;

  for (prescaler = 1; prescaler < 8; prescaler++) { // Find the appropriate prescaler
    top = F_CPU / (long) frequency / (long) _tAC3_prescale[prescaler - 1] - 1; // Calculate the top.
    if (top < 256) break; // Fits, break out of for loop.
  }
  if (top > 255) { noToneAC3(); return; } // Frequency is out of range, turn off sound and return.

  if (length > 0) _tAC3_time = millis() + length; else _tAC3_time = 0xFFFFFFFF; // Set when the note should end, or play "forever".
  ...
  if (length > 0 && !background) { delay(length); noToneAC3(); } // Just a simple delay, doesn't return control till finished.
}
...
ISR(TIMER2_COMPA_vect) { // Timer interrupt vector.
  ...
  if (millis() >= _tAC3_time) {
    noToneAC();
    noToneAC3(); // Check to see if it's time for the note to end.
  } 
